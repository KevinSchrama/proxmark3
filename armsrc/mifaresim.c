    ...
    while ((button_pushed == false) && (finished == false)) {
        ...
        switch (cardSTATE) {
            ...
            case MFEMUL_SELECT: {
                ...
                if (receivedCmd_len == 9 && receivedCmd[1] == 0x70) {
                    if (memcmp(&receivedCmd[2], responses[uid_index].response, 4) == 0) {
                        ...
                        Dbprintf("receivedCmd: %02x %02x %02x %02x %02x %02x %02x %02x %02x", receivedCmd[0], receivedCmd[1], receivedCmd[2], receivedCmd[3], receivedCmd[4], receivedCmd[5], receivedCmd[6], receivedCmd[7], receivedCmd[8]);
                        if (cl_finished) {
                            LED_B_ON();
            ...
            case MFEMUL_WORK: {
                ...
                Dbprintf("MFEMUL_WORK receivedCmd: %02x %02x, cardAUTHKEY %02x", receivedCmd[0], receivedCmd[1], cardAUTHKEY);
                if (encrypted_data) {
                    // decrypt seqence
...
Mifare sim is added to have see how the emulation works step by step.
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
void Mifaresim(uint16_t flags, uint8_t exitAfterNReads, uint8_t *datain, uint16_t atqa, uint8_t sak) {
    tag_response_info_t *responses;
    uint8_t cardSTATE = MFEMUL_NOFIELD;
    uint8_t uid_len = 0; // 4, 7, 10
    uint32_t cuid = 0, selTimer = 0, authTimer = 0;
